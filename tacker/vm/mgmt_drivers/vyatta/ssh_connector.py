...
class SSHConnector():
    ...
    def connect(self, host, user, password):
        try:
            ...
            self.connector.connect(host,username=user,password=password)
            ...
            LOG.exception(_("connect failed to %(host)s with error %(error)s"),
                {'host': host, 'error': e.args})
    ...
    def read_response(self, read_lines=True):
        ...
        :param read_lines: Boolean value which indicates to read multiple line
        or single line.
        :return: Response from the device as list when read_lines is True or
